...
#    'threshold' = normalize based on read counts determined to be from real signal (total is only based on only enriched regions). for samples with non-zero values in the KmetStat column, normalize to these counts; otherwise, normalize to E. coli spike-in DNA
...
manual_analysis_summary = sys.argv[6] # only needed for 'manual' normalization method
...
if norm_method == 'kmet':
	...
	kmet_stats['Scale Factor'] = np.where(kmet_stats['Spike-in Percent']!=0, kmet_stats['Kmet Scale Factor'], kmet_stats['Ecoli Scale Factor'])
	scale_factors = dict(zip(list(kmet_stats['Sample']), list(kmet_stats['Scale Factor'])))

if norm_method == 'manual':
	kmet_stats = pd.read_csv(manual_analysis_summary) #make this file manually by copying kmet_analysis_summary 
	kmet_stats['Manual Scale Factor'] = 1-kmet_stats['Manual _Scale'] #determined using R-script from .bam file and added manually into csv column in metstat analysis output from part 1
	scale_factors = dict(zip(list(kmet_stats['Sample']), list(kmet_stats['Manual Scale Factor'])))
...
for file in os.listdir(picardDir_path):
	if file[-7:] == 'pic.bam':
		...
		if norm_method == 'cpm':
			...
			file_root = '_'.join(fileID_components[:-1]) # CHANGE HERE DEPENDING ON HOW FILES WERE GENERATED IN FIRST PART
			...
			print(' ~ Normalizing deduplicated BAM file based on manual thresholding %s @ %s ~' % (file, norm_time))
			print(' ~ Normalizing deduplicated BAM file based on manual thresholding %s @ %s ~' % (file, norm_time), file=records)
			file_root = '_'.join(fileID_components[:-1]) # CHANGE HERE DEPENDING ON HOW FILES WERE GENERATED IN FIRST PART
			norm_cmd = 'bamCoverage --bam ' + file_path + ' -o ' + dedupfile_path + ' --outFileFormat bedgraph --extendReads --centerReads -p 12 --binSize 10 --scaleFactor ' + str(scale_factors[file_root])
		print(norm_cmd)
